...
public class GuessTheWord {
    ...
    private static class Master {
        ...
        public int guess(String word) {
            Map<Character, Integer> matchesMap = new HashMap<>(occurrences);
            int matches = 0;
            char[] letters = word.toCharArray();
            for(char letter : letters) {
                if(matchesMap.containsKey(letter) && matchesMap.get(key) > 0) {
                    matchesMap.put(letter, matchesMap.get(letter) - 1);
                    matches++;
                }
            }
